...
List_layanan_service = [{"Nomor Member" : 2202185, "Nama" : "Tinky Winky", "Alamat": "Jl. Baru", "Tanggal Pelaporan" : datetime.date(2022,1,5), "Status" : "OPEN"},
                        {"Nomor Member" : 2202301, "Nama" : "Dipsy", "Alamat": "Tanah Sareal", "Tanggal Pelaporan" : datetime.date(2022,1,6), "Status" : "OPEN"},
                        {"Nomor Member" : 2203033, "Nama" : "Lala", "Alamat": "Komp. Yasmin", "Tanggal Pelaporan" : datetime.date(2022, 1, 12), "Status" : "OPEN"},
                        {"Nomor Member" : 2203055, "Nama" : "Asep", "Alamat": "Cimanggu Permai", "Tanggal Pelaporan" : datetime.date(2022,1,14), "Status" : "OPEN"},
                        {"Nomor Member" : 2204055, "Nama" : "Tabi", "Alamat": "Tanah Baru", "Tanggal Pelaporan" : datetime.date(2022,1,17), "Status" : "OPEN"}]

# Menu 1 Menampilkan Daftar Customer
def menudaftar():
    print("\nDaftar Customer: \n")
    print("Nomor Member \t| Nama Customer \t| Alamat \t\t| Tanggal Pelaporan \t| Status")
    for a in range(len(List_layanan_service)) :
        for a in range(0, 5):
            print('{} \t| {}  \t\t| {} \t\t| {} \t\t| {}'.format(List_layanan_service[a]["Nomor Member"], 
                                              List_layanan_service[a]["Nama"],
                                              List_layanan_service[a]["Alamat"], 
...
def menuregis():
    while True:
        ...
        regisalamat = input("Input Alamat: \t")
        ...
        print('{} \t| {} \t| {} \t| {} \t| {}'.format("Nomor Member", "Nama Customer", "Alamat", "Tanggal Pelaporan", "Status"))
        for a in range(len(List_layanan_service)) :
            print('{} \t| {}  \t\t| {} \t| {} \t\t| {}'.format(List_layanan_service[a]["Nomor Member"], 
                                              List_layanan_service[a]["Nama"],
                                              List_layanan_service[a]["Alamat"], 
...
def menubatal():
    ...
    print('{} \t| {} \t| {} \t| {} \t| {} \t| {}'.format("Index", "Nomor Member", "Nama Customer", "Alamat", "Tanggal Pelaporan", "Status"))
    for index, item in enumerate(List_layanan_service):
        print(f"{index} \t| {item['Nomor Member']}  \t| {item['Nama']}  \t\t| {item['Alamat']} \t| {item['Tanggal Pelaporan']} \t\t| {item['Status']}")
...
def menujadwal():              
    ...
    print('{} \t| {} \t| {} \t| {} \t| {} \t| {}'.format("Index", "Nomor Member", "Nama Customer", "Alamat", "Tanggal Service", "Status"))
    for index, item in enumerate(List_layanan_service):
        tanggal = item["Tanggal Pelaporan"]+ datetime.timedelta(days=2)
        print(f"{index} \t| {item['Nomor Member']}  \t| {item['Nama']}  \t\t| {item['Alamat']} \t| {tanggal} \t\t| {item['Status']}")
